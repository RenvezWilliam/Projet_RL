...
def run(episodes = 1000, is_training = True, render = False):
    ...
    # Division de l'espace d'observation en plusieurs sous-parties discrètes
    #
    # Le nombre est volontairement bas car le temps d'apprentissage est deviens rapidement immense
    hull_angle_speed_space      = np.linspace(env.observation_space.low[0], env.observation_space.high[0], 5)
    angular_velocity_space      = np.linspace(env.observation_space.low[1], env.observation_space.high[1], 4)
    horizontal_speed_space      = np.linspace(env.observation_space.low[2], env.observation_space.high[2], 5)
    pos_joint_1_space           = np.linspace(env.observation_space.low[4], env.observation_space.high[4], 5)
    pos_joint_2_space           = np.linspace(env.observation_space.low[6], env.observation_space.high[6], 5)
    touch_ground_space_1        = np.linspace(env.observation_space.low[8], 1.0, 2)
    pos_joint_3_space           = np.linspace(env.observation_space.low[9], env.observation_space.high[9], 5)
    pos_joint_4_space           = np.linspace(env.observation_space.low[11], env.observation_space.high[11], 5)
    ...
    # Division de l'espace d'action en quatres sous-partie
    ...
    # Reformation des quatres sous-parties en une seule pour me simplifier la vie lors de la création de la table
    action_ = []
    for a1 in action_1:
        for a2 in action_2:
            for a3 in action_3:
    ...
    if is_training:
        ...
        f = open('bipedal_walker_ql.pkl', 'rb')
        q = pickle.load(f)
        f.close()
    ...
    rewards_per_episodes = np.zeros(episodes) # Reward per episodes
    ...
    for i in tqdm(range(episodes)):
        ...
        while(not terminated and not truncated):
            ...
            if rng.random() < epsilon and is_training:
                ...
                global_idx = (action_1_idx) * (n2 * n3 * n4) + (action_2_idx) * (n3 * n4) + (action_3_idx) * n4 + (action_4_idx) # action à placer dans la Q-table

            else:
                action_idx = q[shas, sav, shs, spj1, spj2, stg1, spj3, spj4, stg2, l1]

                global_idx = np.argmax(action_idx)
            ...
            if is_training:
                # MISE A JOUR DE LA Q-TABLE -> Q-LEARNING : Q(S,A) = Q(S,A) + α * [reward + γ maxQ(S',A') - Q(S,A)]
                #q[shas, sav, shs, spj1, spj2, stg1, spj3, spj4, stg2, l1, l2, global_idx] += alpha * (reward + gamma * np.max(q[nshas, nsav, nshs, nspj1, nspj2, nstg1, nspj3, nspj4, nstg2, nl1, nl2, :]) - q[shas, sav, shs, spj1, spj2, stg1, spj3, spj4, stg2, l1, l2, global_idx]) 
                q[shas, sav, shs, spj1, spj2, stg1, spj3, spj4, stg2, l1, global_idx] += alpha * (reward + gamma * np.max(q[nshas, nsav, nshs, nspj1, nspj2, nstg1, nspj3, nspj4, nstg2, nl1, :]) - q[shas, sav, shs, spj1, spj2, stg1, spj3, spj4, stg2, l1, global_idx])
            ...
            if reward > best_reward:
                ...
                if is_training:
                    f = open('bipedal_walker_ql.pkl', 'wb')
                    pickle.dump(q, f)
                    f.close()
